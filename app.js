...
(() => {
  ...
  function dashboard() {
    ...
    const body = `
      ...
      <div class="stat-row">
        <div><strong>10</strong><span>units trong sách</span></div>
  ...
  function shortTab(unit) {
    return `<div class="section-intro"><span>03</span><div><h2>Câu hỏi ngắn — tối đa 40 từ</h2><p>Hãy tự viết trước. Website đếm từ và chỉ hiển thị model answer khi bạn chủ động mở.</p></div></div>
  ...
  function essayTab(unit) {
    return `<div class="section-intro"><span>04</span><div><h2>Đề essay và dàn ý</h2><p>Mỗi dàn ý gồm thesis rõ lập trường và bốn bước phát triển lập luận. Yêu cầu mục tiêu: ít nhất 300 từ.</p></div></div>
  ...
  function createExam(scope) {
    ...
    const terms = shuffle(units.flatMap(unit => unit.terms.map((term,index) => ({unitId:unit.id,unitNum:unit.num,index,term:term[0],definition:term[1]})))).slice(0,10);
  ...
  function examPage() {
    ...
    const body = `<div class="exam-live-bar"><div><span>TIME LEFT</span><strong id="examTimer">${formatTime(examSecondsLeft())}</strong></div><div><span>SCOPE</span><strong>${scopeLabel}</strong></div><button class="submit-exam" id="submitExam">Submit test</button></div>
      <form class="exam-paper" id="examPaper">
        <section class="paper-section"><header><div><span>PART 1</span><h2>Terminology</h2></div><strong>3 points</strong></header><p>Write the correct term for each definition.</p>
  ...
  function acceptedTerm(answer, target) {
    ...
    return a === t || a === withoutParen || (acronym && a === normalize(acronym));
  }
  function submitExam(auto = false) {
    if (!state.exam?.active) return;
    clearInterval(timerHandle);
    const exam = state.exam;
    const correctness = exam.terms.map((item,index) => acceptedTerm(exam.termAnswers[index],item.term));
